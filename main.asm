...
main:
	...
	mov		#30, R10							; used to move the cursor around
	mov		#44, R11
	mov		#8, R15
...
while1:
	...
	jnz 	right
	call	handle3					; Yes, branch back and wait

right:
	bit.b	#1, &P2IN					; bit 1 of P1IN set?
	jnz 	left
	call	handleRight

left:
	bit.b	#2, &P2IN					; bit 2 of P1IN set?
	jnz		down
	call 	handleLeft

down:
	bit.b	#4, &P2IN					; bit 4 of P1IN set?
	jnz		up
	call 	handleDown

up:
	bit.b	#5, &P2IN					; bit 5 of P1IN set?
	jnz 	while1
	call	handleUp
...
handle3:
	...
	jz		handle3						; Yes, branch back and wait
	call 	#clearDisplay2
	call 	makeBox
...
setAddress:
	...
	;bis.w	#0xB0, R13			; mask in "B0" as the prefix for a page address... sets the row
	call	#writeNokiaByte
